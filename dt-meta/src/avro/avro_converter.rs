...
impl AvroConverter {
    ...
    pub fn avro_value_to_row_data(&self, mut payload: &[u8]) -> Result<RowData, Error> {
        let value = from_avro_datum(&self.schema, &mut payload, None)?;
    ...
    fn test_row_data_to_avro() {
        ...
        let validate = |row_data: RowData| {
            let payload = avro_converter
                ...
                .unwrap();
            let decoded_row_data = avro_converter
                .avro_value_to_row_data(payload.as_slice())
                .unwrap();
